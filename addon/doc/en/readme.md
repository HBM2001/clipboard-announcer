...
Open the context menu with the Applications key, `Shift+F10`, or a right click to pin or unpin an item, edit text items, or delete an item. You can also press `Delete` to remove the selected item. Pinned items stay at the top of the history and are kept when you choose `Clear all history`; deleting a pinned item requires confirmation. Use `Move pinned item up` and `Move pinned item down` to choose their persistent order, or press `Ctrl+Page Up` and `Ctrl+Page Down` to move the selected pinned item.
